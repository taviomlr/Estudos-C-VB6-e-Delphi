...
Public Property Let Tipo(ByVal nTipo As Byte)
    ...
    'Só habilita a atualização dos saldos bancários no Extrato Bancário
    'pt. 88218 - Ivo Sousa (23/09/2008) - Deivar o campo desabilitado
    'chkFluxo(0).Visible = (mbitTipo = TIPO_EXTRATO) Or (mbitTipo = TIPO_MOVIMENTO)
...
Dim strFoxIni As String
  ...
  ' Se o usuário não deseja controlar centro de custo não exibe o controle
  ' na tela
  '
...
Private Function EnableUpdate() As Boolean
  ...
  'Deixar habilitado o campo atualizar saldo para permitir ao sistema atualizar o saldo
  'retirado o criterio (chkFluxo(4).Value = vbChecked)
...
Private Sub FluxoMsgBar(intTabIndex As Integer)
  ...
  '
  ' Caixa de verificação de atualização do Saldo
  Case 5
    MsgBar "Marque para atualizar os Saldos Bancários"
  '
  ' Caixa de verificação Quebrar Por Bancos
  Case 6
    MsgBar "Quebra o relatório por Banco"
  '
  ' Caixa de verificação Imprimir Descrição completa
  Case 7
    MsgBar LoadResString(176)
  '
  ' Caixa de verificação Imprimir Resumo
  Case 8
    MsgBar LoadResString(177)
  '
  ' Caixa de verificação Imprimir Razão
  Case 9
    MsgBar LoadResString(175)
  '
  ' Campos de Códigos de Bancos
  Case 12, 15
    MsgBar LoadResString(152) & ResolveResString(75, resUM, "Bancos")
  '
  ' Campos de Códigos de Contas
  Case 19, 22
...
Private Sub CriaFiltroFluxo(pdPraOnde As PrintDestinoEnum)
    ...
    Dim strCCusto    As String        'Centro de Custo
    ...
    If (mbitTipo = TIPO_EXTRATO Or mbitTipo = TIPO_MOVIMENTO) Then                  '// Quando o tipo for extrato bancário
        ...
        Else
            ...
            strSe(5) = "Banco = <Banco> AND (CONVERT(varchar(10),[Liberação],120) = " & Quote("<Data>", "''") & _
                       ...
                       "'P'" '// Tabela de Lançamentos ou Duplicatas Pagas
        End If
    'Se Tipo for Fluxo de Caixa obtém todos os dados, mesmo os não pagos
    Else
        #If FOXSQL = 1 Then
    ...
    If (AbreRecordset(rstBancos, strBancos, dbOpenSnapshot) = WL_OK) Then
        'Se criar as tabelas temporárias
        If (CrieTemp(rstTemp) And TempSaldos(rstSaldos)) Then
...
Dim MaisUm    As Boolean              '// Controle para banco zerado
  ...
  rstBcos.MoveFirst                 '// Move para o primeiro registro
  Do
        ...
        For i = 0 To rstData.Fields.Count - 1
          Select Case TransDBTypeRetInt(rstData(i).Type)
            Case dbText
                ...
                'Pt. 95943 - Moacir Pfau(17/11/2009)
                If gTipoDB = Access Then
            ...
            '
            If rstData(i).name = "Empresa" Then rstData(i).value = Space(1)
    ...
    Do Until (DateDiff("d", DtDia, DtData(1)) < ZERO)   '// Até que dtDia seja posterior a dtData(1)
      ...
      DtDia = DateAdd("d", 1, DtDia)      '// Soma um dia a data atual
    Loop
...
AddSintetico_Erro:
  ...
  If (err.Number) Then
    If Not rstSld.EOF Or Not rstSld.BOF Then
        ...
        End If
    End If
    DAOErros erro(err.Number) & " AddSinteticoA"
  'Else *** deve gerar uma excecao abortando toda a cascata de eventos que trouxe o codigo ateh aqui
    'Exit Function
  End If
  Resume
End Function
...
Dim strConciliado As String
  ...
  If chkFluxo(6).value = vbChecked Or _
       ...
       chkFluxo(8).value = vbChecked Or _
       chkFluxo(9).value = vbChecked Then
    cDebito = cDebito + Round(Soma("Valor", NomeTabeladoRST(rstPrevisao), Replace(strWhere, "Liberação", "Vencimento", , , vbTextCompare), ZERO) / UltimaCotacao(txtFluxo(7).Text, DtData), 2)
  End If
...
On Error GoTo AddAnaliticoA_Erro
    ...
    Do
        ...
        rstSaldos("Tipo").value = False   'False é usado para identificar saldo inicial
        rstSaldos("Valor").value = cSaldo
        rstSaldos.update
        
        'pt. 96391 - Ivo Sousa(21/12/2009) - Incluido o Sleep para atualização do saldo na tabela temporária
        Call Sleep(500)
        ...
        Do Until (DateDiff("d", DtDia, dtDatas(1)) < ZERO)  '// Até que dtdia seja posterior a data final
            If (mbolCancelou) Then
                GoTo AddAnaliticoA_Erro
            End If
            DoEvents
            SimpleMsgBar ResolveResString(1023, _
                                          resUM, CStr(GetValue(rstBancos, "Banco", ZERO)), _
            ...
            rstSaldos("Tipo").value = True        '// True é usado para identificar o saldo final
            rstSaldos("Valor").value = cSaldo
            rstSaldos.update
...
Dim dblCotMoedaBase As Double   '// Cotação da moeda base
  ...
  If (AbreRecordset(rstDados, strInst, dbOpenForwardOnly) = WL_OK) Then
    Do Until (rstDados.EOF)     '// Loop até o final do Recordset
      ...
      End If
      ' 24/08/2018 - D. 233247 - H. 233248 - Yuji F. - Ajuste na busca do tipo de registro para transferências bancárias
      rstTemp("Tipo").value = GetValue(rstDados, IIf(lngTipo > 1, "Tipo", "Tipo_registro"), NUL)
      ...
      End If
        ...
        If UCase(GetValue(rstDados, "Moeda", ZERO)) <> UCase(txtFluxo(7).Text) Then
           If Not IsNull(rstTemp("Pagamento").value) Then
              dblCotMoedaDoc = UltimaCotacao(GetValue(rstDados, "Moeda"), rstTemp("Pagamento").value, True)
              dblCotMoedaBase = UltimaCotacao(txtFluxo(7).Text, rstTemp("Pagamento").value, True)
           Else
              dblCotMoedaDoc = UltimaCotacao(GetValue(rstDados, "Moeda"), rstTemp("Data").value, True)
              dblCotMoedaBase = UltimaCotacao(txtFluxo(7).Text, rstTemp("Data").value, True)
           End If
...
Dim bQuebra         As Boolean      'Define se há quebra por banco ou não
  ...
  With wrkAnalitico
    ...
    With .Grupo(1).Subgrupo(1).Header.Linha(4)
      ...
      .AddCampo , wrCSFixedText, "Código", , 16
